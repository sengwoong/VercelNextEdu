...
function Messagecomponents({ message ,user}: Props) {
  ...
  useEffect(() => {
    if (chatRef.current) {
      ...
      if (chatContainer) {
        if (chatContainer.scrollTop + (chatContainer.scrollTop / 3) > chatContainer.scrollHeight || chatContainer.scrollTop === 0) {
    ...
    >
      ...
      <div>
        ...
        <div className="flex items-end">
          <div className={`px-3 py-2 rounded-lg w-fit text-white ${isUser ? "bg-blue-400" : "bg-red-400"}`}>
            <p>{message.message}</p>
          </div>
